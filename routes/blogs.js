        ...
        if(!req.params.id){
            ...
            Blog.findOne({ _id: req.params.id}, (err, blog) => {
                if(err){
                    ...
                    if (!blog){
                        res.json({ success: false, message: 'Blog not founf.'});
                    }else {
                        ...
                        User.findOne({ _id: req.decoded.userId }, (err, user ) => {
                            if(err) {
                                ...
                                if(!user){
                                    ...
                                    if(user.username !== blog.createdBy) {
                                        res.json({ success:false, message: 'your are not authorized to delete this blog post '});
                                    } else {
                                        blog.remove((err) =>{
                                            if(err) {
                                                res.json({ success: false, message: err });
                                    ...
                                    if (user.username === blog.createdBy) {
                                        ...
                                        if(blog.likedBy.includes(user.username)){
                                            ...
                                            if (blog.dislikedBy.includes(user.username)){
                                                ...
                                                blog.save((err) => {
                                                    if (err){
                                                        res.json({ success: false, message: 'SomeThing went wrong in it'});
                                                    } else {
                            ...
                            User.findOne({ _id: req.decoded.userId }, (err, user) =>{
                                if(err) {
                                    ...
                                    if (!user){
                                        ...
                                        blog.save((err) => {
                                            if(err) {
                                                res.json({ success: false, message: 'Something went wornt'})
                                            } else {
